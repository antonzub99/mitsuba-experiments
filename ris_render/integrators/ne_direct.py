...
class NeIntegrator(mi.SamplingIntegrator):
    def __init__(self, props=mi.Properties()):
        ...
        self.m_weight_bsdf = 1. / self.bsdf_samples
        self.m_weight_em = 1. / self.emitter_samples
        self.m_frac_bsdf = self.m_weight_bsdf / self.ttl_samples
        self.m_frac_em = self.m_weight_em / self.ttl_samples
        
        
        
    def _render(self: mi.SamplingIntegrator,
               scene: mi.Scene,
               sensor: mi.Sensor,
               seed: int = 0,
               spp: int = 0,
    ...
    def sample(self,
        ...
        L += dr.select(active, emitter_vis.eval(si), mi.Color3f(0))
        ...
        """
        loop = mi.Loop(name="Emitter sampling", 
                       state=lambda: (sampler, si, active_em, L))
        loop.set_max_iterations(self.emitter_samples)

        while loop(active_em): 
        """
